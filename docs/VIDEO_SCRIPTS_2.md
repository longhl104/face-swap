...
Generator không giữ nguyên từng điểm ảnh rồi chỉnh từng chỗ như Photoshop. Một bức ảnh có quá nhiều điểm ảnh để mạng xử lý theo kiểu đó. Cách phổ biến là mô hình **nén** ảnh xuống thành một bản tóm tắt nhỏ — chỉ giữ những gì quan trọng như “đang cười, quay trái, ánh sáng thế nào” — rồi từ bản tóm tắt đó **vẽ lại** khuôn mặt mới. Có thể hình dung giống gửi ảnh qua tin nhắn với dung lượng rất thấp: máy phải nén mạnh rồi mới mở lại; nội dung vẫn nhận ra được, nhưng cạnh mắt, mép miệng dễ bị nhòe hoặc lệch chỗ.

Nếu chỉ nén rồi khôi phục một cách thô, danh tính có thể còn, nhưng vị trí mắt, miệng và cấu trúc khuôn mặt dễ mất. Vì vậy Generator của chúng ta dùng kiến trúc **U-Net**: các đường nối tắt giữa phần nén và phần vẽ lại — giống như vừa gửi bản tóm tắt, vừa kèm theo vài “ảnh phác thảo” độ phân giải cao hơn để khi vẽ lại vẫn biết mắt và miệng nằm đâu. Kiến trúc này vốn được đề xuất để khoanh vùng tế bào trên ảnh kính hiển vi trong y học; về sau được dùng rộng trong các bài toán ảnh, kể cả face swap. Chúng ta sẽ giải thích kỹ hơn về U-Net ở các phần sau của series.

Đó là hai thành phần chính trong mô hình của chúng ta — căn bản để bắt đầu. Hệ thống còn có thêm Discriminator và quá trình huấn luyện với nhiều hàm mất mát hay còn gọi là loss functions; những phần đó sẽ được nói kỹ ở các video sau. Phần này tập trung vào việc chuẩn bị: chọn dữ liệu và đưa ảnh về dạng mô hình có thể học được.
...
Trước khi nói về dữ liệu, cần biết cấu hình máy chúng ta dùng để huấn luyện: GPU NVIDIA RTX 5070 Laptop với 8 GB VRAM. Giới hạn bộ nhớ này sẽ ảnh hưởng trực tiếp đến các lựa chọn phía sau.
...
Vậy tại sao không để nguyên ảnh rồi chỉ đổi sang `.npy`? Vì `.npy` chỉ là cách lưu mảng số cho nhanh khi huấn luyện. Đổi định dạng không tự cắt mặt, không tự căn giữa, cũng không đưa mọi mẫu về cùng một kích thước. Không qua `FacePreprocessor`, file `.npy` vẫn chỉ là ảnh gốc dưới dạng khác — còn thiếu đúng thứ mô hình cần: một vùng khuôn mặt sạch, vuông và thống nhất.
